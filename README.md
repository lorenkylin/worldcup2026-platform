...
> **文档版本**：v0.15.0（**深空玻璃拟态 UI 视觉升级** —— 国际字体/玻璃拟态/微光边框/动效 + API-Football 主源 + worldcup26.ir 备份 + 字段级多源仲裁，2026-06-18）
> **阶段**：Phase 5 – Ship ✅ **完成**（v0.7.x 模型演进 + 赔率深化 + 缓存 + Adaptive Weight + 数据回填 + 校准实验 4 版 + Cockpit 速览 + 校准 sunset 决策 + **v0.14 多源数据接入 + v0.14.1 数据质量校验层 + v0.14.2 Cockpit 去重 redesign + v0.14.3 Cockpit 入口下线/数据同步稳定性修复/API-Football 预算告警 + v0.14.4 字段级多源仲裁 + v0.15.0 深空玻璃拟态 UI 升级**）
...
## 六、API 速查（v0.15.0 共 68 端点，含 `/`、`/health`、`/api/*`）
