...
> ## 🏠 **v0.16.0 — 首页信息架构重构 + 国旗/中文文案优化（2026-06-18）**
> 首页从“小组积分榜 + 焦点战”拆分为“焦点战预测卡片 + 热门投注推荐”两大核心模块：移除小组 mini 积分榜避免与积分页重复，扩展焦点战展示赔率趋势、模型预测、状态提醒与快捷入口，新增基于模型价值投注的热门 3 场推荐列表；同步优化全站国旗显示逻辑与中文文案一致性。Service Worker 缓存版本升至 `wc2026-v5`，静态资源加 `?v=0.16.0`。
>
...
> **文档版本**：v0.16.0（**首页信息架构重构 + 国旗/中文文案优化** —— 焦点战/热门投注 + API-Football 主源 + worldcup26.ir 备份 + 字段级多源仲裁 + 深空玻璃拟态 UI，2026-06-18）
> **阶段**：Phase 5 – Ship ✅ **完成**（v0.7.x 模型演进 + 赔率深化 + 缓存 + Adaptive Weight + 数据回填 + 校准实验 4 版 + Cockpit 速览 + 校准 sunset 决策 + **v0.14 多源数据接入 + v0.14.1 数据质量校验层 + v0.14.2 Cockpit 去重 redesign + v0.14.3 Cockpit 入口下线/数据同步稳定性修复/API-Football 预算告警 + v0.14.4 字段级多源仲裁 + v0.15.0 深空玻璃拟态 UI 升级 + v0.16.0 首页重构/国旗/中文优化**）
...
| ✅ 自动化测试 | **542 项**单元 + 集成（实测：542 passed, 1 skipped）+ **69 项** Playwright E2E（实测：69 passed），全部通过 |
...
## 六、API 速查（v0.16.0 共 68 端点，含 `/`、`/health`、`/api/*`）
