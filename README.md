...
> ## 🏠 **v0.16.3 — 推荐比分 UI 醒目化（2026-06-24）**
> 在 v0.16.2 基础上对推荐比分的视觉权重做进一步升级：首页焦点战卡片、比赛详情页 AI 预测区、赛后复盘模块的首选比分使用 amber 高亮 + 加粗大字号，次选比分使用 emerald 区分，让用户在深色背景下也能一眼捕捉核心预测信息。Service Worker 缓存版本升至 `wc2026-v7`，静态资源加 `?v=0.16.3`。
>
...
> 对 H5 SPA 进行全面视觉重塑：深空沉浸背景 + 玻璃拟态卡片（Glassmorphism）+ 微光边框 + Inter / Noto Sans SC 国际字体组合；悬浮胶囊 Tab、抽屉菜单、页面进入缓动动效、实时比分霓虹指示器；所有页面（赛程/积分榜/球队/比赛详情/赔率/Elo/历史交锋/出线模拟器/准确率/数据源健康）统一升级配色与排版，保持原有 data-testid 与业务逻辑零侵入。Service Worker 缓存版本升至 `wc2026-v4`，静态资源加 `?v=0.15.0`。
>
> ## ⚖️ **v0.14.4 — 多源字段级仲裁（2026-06-18）**
> 新增字段级置信度仲裁器 `app/services/field_arbiter.py` + 协调器 `app/services/multi_source_arbitration.py`：API-Football 与 worldcup26.ir 对同场比赛的每个字段按置信度/时效/状态机/源优先级做仲裁，冲突写入 `data/field_arbitration_log.jsonl`；manual 源永不覆盖、已结束比分不回退、状态只允许 `scheduled → live → finished` 正向推进。`app/services/multi_source_sync.py` 在 API-Football 主源可用时先同步非比赛数据，再与 worldcup26.ir 做字段级仲裁；新增只读管理端点 `GET /api/admin/sync/arbitration` 预览仲裁结果。
> **542 passed, 1 skipped 全绿**（含 20 项字段仲裁单元测试）。
>
> ## 🧹 **v0.14.3 — Cockpit 驾驶舱入口下线 + 数据同步稳定性修复 + API-Football 预算告警（2026-06-18）**
> 从 SPA 底部 Tab 移除 Cockpit 入口及关联渲染代码，避免已下线的总览驾驶舱继续占用前端体积和导航位；Service Worker 缓存版本升到 `wc2026-v3`，静态资源 `?v=0.14.3`。
> 后端同步修复 worldcup26.ir 脏数据导致的 `ck_standings_match_count` 同步失败（`standings` 数值冲突时自动标准化）；新增 `app/services/budget_alert.py` 预算告警，API-Football 日调用量达到 80%/95% 时通过邮件/企业微信通知，状态持久化 + 去重 + `/api/health/sources` 暴露预算详情。
> **522 passed, 1 skipped 全绿**。
>
...
> **494 passed, 1 skipped + 56 E2E 全绿**。
>
...
> **文档版本**：v0.16.3（**推荐比分 UI 醒目化** —— 首页焦点战/比赛详情/赛后复盘首选 amber 高亮、次选 emerald 区分 + 比分推荐 v2.1 + API-Football 主源 + worldcup26.ir 备份 + 字段级多源仲裁 + 深空玻璃拟态 UI，2026-06-24）
> **阶段**：Phase 5 – Ship ✅ **完成**（v0.7.x 模型演进 + 赔率深化 + 缓存 + Adaptive Weight + 数据回填 + 校准实验 4 版 + Cockpit 速览 + 校准 sunset 决策 + **v0.14 多源数据接入 + v0.14.1 数据质量校验层 + v0.14.2 Cockpit 去重 redesign + v0.14.3 Cockpit 入口下线/数据同步稳定性修复/API-Football 预算告警 + v0.14.4 字段级多源仲裁 + v0.15.0 深空玻璃拟态 UI 升级 + v0.16.0 首页重构/国旗/中文优化 + v0.16.1 比分推荐 v2.1 + v0.16.2 前端首选/次选比分展示优化 + v0.16.3 推荐比分 UI 醒目化**）
...
| ✅ 后端 API（FastAPI · **68 端点**） | matches (4) / teams (3) / groups (1) / predictions (2) / elo (**19**,含 Glicko-2/Blend/Adaptive/Weight Sweep/Live Accuracy/已 sunset 的 calibration 410 端点) / h2h (2) / simulator (2) / bracket (1) / odds (**10+**) / weight-sweep (1) / admin (4) / admin_sync (**10**,含字段仲裁预览) / admin_odds (4) / health (1) + `/api/health/*` (3) |
...
| ✅ 自动化测试 | **542 项**单元 + 集成（实测：542 passed, 1 skipped）+ **56 项** Playwright E2E（实测：56 passed），全部通过 |
...
worldcup26_sync.py ──→ SQLite (48 队 + 16 球场 + 104 比赛 + 48 standings)
                          ↑
multi_source_arbitration.py ←── 字段级置信度仲裁（API-Football vs worldcup26.ir）
   ↓ 按字段置信度/时效/状态机选出最优值，冲突写入 field_arbitration_log.jsonl
                          ↑
...
Admin POST (X-Admin-Token) ──→ admin.py + admin_sync.py
                          ↓
                   FastAPI API (68 端点，含 /、/health、/api/cockpit/summary、/api/admin/sync/arbitration)
                          ↓
...
## 六、API 速查（v0.16.3 共 68+ 端点，含 `/`、`/health`、`/api/*`）
...
| 风险 | 状态 | 兜底 |
|---|---|---|
| API-Football 100 次/天不够 | 🟢 已监控告警 | 日预算守护 + 80%/95% 阈值邮件/企微告警 + worldcup26.ir 备份 + worldcupstats.football 爬虫 + 人工兜底 |
...
| 2026-06-18 | **v0.14.3** | **Cockpit 驾驶舱入口下线 + 数据同步稳定性修复 + API-Football 预算告警**：(1) 前端 `app/static/js/app.js` 移除底部 Tab Cockpit 入口及 `renderCockpit` 关联代码，`app/static/sw.js` 缓存版本升 `wc2026-v3`，`index.html` 静态资源加 `?v=0.14.3`；(2) 修复 `app/services/worldcup26_sync.py` `_normalize_standing_values()` 对 worldcup26.ir 脏数据（如 `w=3, mp=1`）的标准化，避免 `ck_standings_match_count` CHECK 约束导致同步连续失败；(3) 新增 `app/services/budget_alert.py` + `tests/test_budget_alert.py` 13 单元测试，API-Football 日调用量达到 80%/95% 时通过邮件 SMTP / 企业微信 webhook 告警，状态持久化到 `data/budget_alert_status.json`，按 UTC 日期 + 告警级别去重，`/api/health/sources` 暴露预算详情；(4) 更新 `app/config.py` / `.env.example` / `deploy_checklist.md` 预算告警配置；(5) 重启 uvicorn 加载最新代码，数据同步从 `stale` 恢复为 `fresh`；(6) 同步 README / `docker-compose.yml` / `deploy_checklist.md` 版本号到 `0.14.3`；(7) commit `1def935` / `5e97527` + **git tag v0.14.3**；**522 passed, 1 skipped 全绿** |
| 2026-06-18 | **v0.15.0** | **深空玻璃拟态 UI 视觉升级**：(1) 前端 `app/static/index.html` 引入 Inter / Noto Sans SC 字体、theme-color、悬浮胶囊 Tab、抽屉菜单、静态资源加 `?v=0.15.0`；(2) `app/static/css/styles.css` 新增深空背景、玻璃拟态 `glass-card`、微光边框 `glow-border`、页面进入/卡片悬停/实时霓虹等动效 Token；(3) `app/static/js/app.js` 统一升级 `matchCard` / `statusBadge` / `score-big` 与所有主要页面渲染（赛程/积分榜/球队/比赛详情/赔率/模拟器/Elo/历史交锋/准确率/数据源健康）为玻璃卡片 + 渐变标题条 + 发光空状态，保留全部 data-testid 与业务逻辑；(4) `app/static/sw.js` 缓存版本升 `wc2026-v4`，`manifest.json` 同步 v0.15.0；(5) 更新 README 顶部 banner、API 速查版本、§10 变更日志；(6) **测试目标：542 passed, 1 skipped 全绿** |
| 2026-06-18 | **v0.16.0** | **首页信息架构重构 + 国旗/中文文案优化**：(1) 首页移除小组 mini 积分榜，避免与积分页重复；(2) 扩展焦点战模块：展示实时/ upcoming 焦点比赛、赔率走势、模型预测、状态提醒与快捷入口；(3) 新增“热门投注推荐”区域，基于模型价值投注筛选 Top 3 比赛；(4) 全站国旗显示逻辑与中文文案一致性优化；(5) `app/static/sw.js` 缓存版本升 `wc2026-v5`，`index.html` 静态资源加 `?v=0.16.0`；(6) `app/main.py` 版本常量同步 `0.16.0`；(7) 更新 README banner/文档版本/§10 变更日志；(8) **598 passed, 1 skipped 全绿** |
| 2026-06-24 | **v0.16.1** | **比分推荐 v2.1 上线 + 全量验收通过**：(1) `app/services/prediction.py` 新增首选比分 `primary_score`、次选比分 `secondary_score`、比分推荐度星级 `score_reliability_stars`；(2) 完善推荐原因文案，包含首选/次选与星级说明；(3) `app/services/backtest.py` 回测报告新增首选/次选比分命中率；(4) `app/schemas.py` / `app/models.py` / `app/services/prediction_cache.py` 扩展对应字段，新增 2 条 Alembic migration；(5) 更新 `tests/test_prediction.py`、`tests/test_api.py`、`tests/test_backtest.py`、`tests/test_edge_cases.py`；(6) `app/main.py` 版本常量升 `0.16.1`，README banner/文档版本/API 速查/§10 变更日志同步；(7) 生成 `deliverables/acceptance_report_2026-06-24.md`；(8) **545 passed, 1 skipped + 56 E2E 全绿**，核心 API 19+ 端点 200 无 500 |
| 2026-06-24 | **v0.16.2** | **前端首选/次选比分展示优化**：(1) `app/static/js/app.js` 首页焦点战卡片、比赛详情页 AI 预测区、赛后复盘模块同步展示 `primary_score` / `secondary_score`；(2) 保留 `recommended_score` 回退；(3) `app/static/index.html` 静态资源 query string 升 `?v=0.16.2`；(4) `app/main.py` 版本常量升 `0.16.2`，README banner/文档版本/API 速查/§10 变更日志同步；(5) 新增 `deliverables/v0.16.2_spec.md` & `v0.16.2_release.md`；(6) **545 passed, 1 skipped + 56 E2E 全绿** |
| 2026-06-24 | **v0.16.3** | **推荐比分 UI 醒目化**：(1) `app/static/js/app.js` 三处推荐比分展示使用 amber/emerald 高亮样式——比赛详情页 AI 预测区标题改为 badge 高亮，首页焦点战卡片首选比分 amber-300 大字号、次选 emerald-300，赛后复盘卡片预测比分 amber-300 高亮；(2) `app/static/index.html` 静态资源 query string 升 `?v=0.16.3`；(3) `app/static/sw.js` 缓存版本升 `wc2026-v7`；(4) `app/main.py` 版本常量升 `0.16.3`，README banner/文档版本/API 速查/§10 变更日志同步；(5) 新增 `deliverables/v0.16.3_spec.md` & `v0.16.3_release.md`；(6) **545 passed, 1 skipped + 56 E2E 全绿** |
