...
> ## 🧹 **v0.14.3 — Cockpit 驾驶舱入口下线 + 底部导航精简（2026-06-18）**
> 从 SPA 底部 Tab 移除 Cockpit 入口及关联渲染代码，避免已下线的总览驾驶舱继续占用前端体积和导航位；Service Worker 缓存版本升到 `wc2026-v3`，静态资源 `?v=0.14.3`。
> 后端同步修复 worldcup26.ir 脏数据导致的 `ck_standings_match_count` 同步失败（`standings` 数值冲突时自动标准化）。
> **509 passed, 1 skipped 全绿**。
>
...
> **文档版本**：v0.14.3（**Cockpit 入口下线 + 数据同步稳定性修复** —— API-Football 主源 + worldcup26.ir 备份 + football-data.org 增强 + 多源回退 + 去重/时效/状态机/优先级保护 + 总览聚合 API，2026-06-18）
> **阶段**：Phase 5 – Ship ✅ **完成**（v0.7.x 模型演进 + 赔率深化 + 缓存 + Adaptive Weight + 数据回填 + 校准实验 4 版 + Cockpit 速览 + 校准 sunset 决策 + **v0.14 多源数据接入 + v0.14.1 数据质量校验层 + v0.14.2 Cockpit 去重 redesign + v0.14.3 Cockpit 入口下线/数据同步稳定性修复**）
...
| 2026-06-18 | **v0.14.3** | **Cockpit 驾驶舱入口下线 + 数据同步稳定性修复**：(1) 前端 `app/static/js/app.js` 移除底部 Tab Cockpit 入口及 `renderCockpit` 关联代码，`app/static/sw.js` 缓存版本升 `wc2026-v3`，`index.html` 静态资源加 `?v=0.14.3`；(2) 修复 `app/services/worldcup26_sync.py` `_normalize_standing_values()` 对 worldcup26.ir 脏数据（如 `w=3, mp=1`）的标准化，避免 `ck_standings_match_count` CHECK 约束导致同步连续失败；(3) 重启 uvicorn 加载最新代码，数据同步从 `stale` 恢复为 `fresh`；(4) 同步 README / `docker-compose.yml` / `deploy_checklist.md` 版本号到 `0.14.3`；(5) commit `1def935` + **git tag v0.14.3**；**509 passed, 1 skipped 全绿** |
